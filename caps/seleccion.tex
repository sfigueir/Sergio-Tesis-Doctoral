...
Cuando $\M$ es una máquina autodelimitante universal, hablaremos directamente de complejidad de Kolmgorov libre de prefijos y la denotaremos como $\K$. La complejidad $\K$ está relacionada con la probabilidad de que una máquina libre de prefijos produzca una cadena dada. Esta probabilidad se define como \glossary{$P$}:
...
\begin{teorema}[\index{Coding Theorem}Teorema de Codificación]\label{intro:thm:coding} Para cada máquina libre de prefijos $\M$, se puede encontrar una constante $c$ tal que $ (\forall
